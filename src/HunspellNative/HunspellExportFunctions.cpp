...
{
	char * word_buffer = ((NHunspell *) handle)->GetWordBuffer(word);
	bool correct = ((NHunspell *) handle)->spell(std::string(word_buffer)) != 0;
	return correct;
}
...
DLLEXPORT void * HunspellSuggest(NHunspell * handle, wchar_t * word )
{
	...
	// Calculation of the Marshalling Buffer. 
...
DLLEXPORT void * HunspellAnalyze(NHunspell * handle, wchar_t * word )
{
	...
	// Calculation of the Marshalling Buffer. Layout: {Pointers- zero terminated}{Wide Strings - zero terminated}
